...
export const Editar = async (usr: Usuario, id: number): Promise<boolean> => {
    try {
        // Corrección de la consulta SQL, eliminando la coma extra antes de WHERE
        let tsql = `UPDATE Usuario SET Nombre='${usr.nombre}', Correo='${usr.correo}', Contrasena='${usr.contrasena}' WHERE id=${id}`;
