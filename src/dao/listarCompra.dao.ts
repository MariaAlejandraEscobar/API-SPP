...
export const eliminar = async (id: number): Promise<boolean> => {
    try {
        const tsql = `DELETE FROM ListarCompra WHERE idListarCompra = ${id}`;
        const pool = await GetConnection();
        const rs = await pool.query(tsql);
        return rs.rowsAffected.length === 1;
    } catch (error) {
...
export const editar = async (listarCompra: ListarCompra, id: number): Promise<boolean> => {
    try {
        const tsql = `UPDATE ListarCompra SET idCompra = ${listarCompra.idCompra}, idProducto = ${listarCompra.idProducto}, cantidad = ${listarCompra.cantidad} WHERE idListarCompra = ${id}`;
        const pool = await GetConnection();
        const rs = await pool.query(tsql);
        return rs.rowsAffected.length === 1;
    } catch (error) {
